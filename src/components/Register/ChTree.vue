      ...
      <div class="card-header pb-0">
        <div class="d-flex justify-content-between">
          <h4 class="card-title mg-b-0" style="padding-top: 10px !important;">
            چارت های سازمانی
            <span v-if="ChViewList_Value" class="badg_select">
              {{ ChViewList_Value.persianTitle }}
            </span>
          </h4>
...
async function index() {
  loading.value = true;
  try {
    const response = await ChartService.GetChartTreeWithOrganizationId(useLocalStorageService.getTreeSelectedItem('OrganizationViewList_ModalCreate').Id);
    console.log('=========================>' +useLocalStorageService.getTreeSelectedItem('OrganizationViewList_ModalCreate').Id);
...
const ChViewList_Value = computed(() =>
